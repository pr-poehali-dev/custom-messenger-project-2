  ...
  return (
    <div className="h-screen w-screen flex overflow-hidden relative bg-background">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div
          className="float-orb absolute top-[-10%] left-[-5%] w-96 h-96 rounded-full blur-[80px]"
          style={{ background: "rgba(124, 58, 237, 0.18)" }}
        />
        <div
          className="float-orb absolute bottom-[-10%] right-[-5%] w-80 h-80 rounded-full blur-[80px]"
          style={{ background: "rgba(56, 189, 248, 0.15)", animationDelay: "3s" }}
        />
        <div
          className="float-orb absolute top-[40%] right-[30%] w-64 h-64 rounded-full blur-[60px]"
          style={{ background: "rgba(244, 114, 182, 0.08)", animationDelay: "1.5s" }}
        />
      </div>
      ...
      <aside className="w-80 flex-shrink-0 flex flex-col glass border-r border-white/5 relative z-10">
        ...
        <div className="p-5 flex items-center justify-between">
          <h1 className="font-display font-black text-2xl gradient-text tracking-tight">
            Поток
          </h1>
        ...
        <div className="px-4 pb-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/8 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white/80 placeholder:text-white/30 focus:outline-none focus:border-violet-500/50 focus:bg-white/8 transition-all"
            />
          </div>
        </div>
              ...
              <div className="relative flex-shrink-0">
                <div
                  className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${
                    AVATAR_COLORS[chat.avatar] || "from-violet-500 to-blue-400"
        ...
        <div className="p-4 border-t border-white/5">
          <div className="flex items-center gap-3 p-2 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500 to-pink-500 flex items-center justify-center text-xs font-bold text-white">
              ВЫ
            </div>
            ...
            <div className="relative">
              <div
                className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${
                  AVATAR_COLORS[activeChat.avatar] || "from-violet-500 to-blue-400"
                } flex items-center justify-center text-sm font-bold text-white shadow-lg`}
              >
                {activeChat.isGroup ? (
          ...
          {currentMessages.map((msg, i) => (
            <div
              ...
              className={`flex animate-fade-in ${
                msg.isOut ? "justify-end" : "justify-start"
              }`}
            ...
            >
              ...
              <div className="max-w-[65%]">
                {!msg.isOut && activeChat.isGroup && msg.author && (
                  <p className="text-[11px] text-violet-400 font-medium mb-1 pl-1">
                    {msg.author}
                  </p>
                )}
                <div
                ...
                <p
                  className={`text-[10px] text-white/25 mt-1 ${
                    msg.isOut ? "text-right pr-1" : "pl-1"
                  }`}
                >
                  {msg.time}
                </p>
          ...
          <div className="flex items-end gap-3">
            ...
            <div className="flex-1 relative">
              <input
                ...
                className="w-full bg-white/5 border border-white/8 rounded-2xl px-4 py-3 text-sm text-white/90 placeholder:text-white/25 focus:outline-none focus:border-violet-500/60 focus:bg-white/8 transition-all pr-12"
              />
            ...
            >
              <Icon
                name="Send"
                size={16}
                className={inputValue.trim() ? "text-white" : "text-white/25"}
              />
            </button>
          </div>
        </div>
